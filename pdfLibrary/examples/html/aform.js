...
function getField(field) {
	...
	field = field.split(" ").join("_"); // fix for spaces in field names since we make them underscores in our CSS
...
// Reset the HTML form components
function resetForm(shouldExclude, resetfields) {
	if(fields === undefined) {
		fields = getFields(); // lazy init of fields
	}
	if(shouldExclude) {
		for(var i = 0; i < fields.length; i ++) {
			var field = fields[i];
			var present = false;
			for(var y = 0; y < resetfields.length; y ++) {
				var name = resetfields[y];
		...
		// if given blank resetFields array reset all fields
		if(resetfields === undefined) {
			resetfields = new Array();
			for(var i = 0; i < fields.length; i ++) {
				resetfields.push(fields[i].name);
			}
		}

		for(var i = 0; i < resetfields.length; i ++) {
			var name = resetfields[i];
...
if(typeof HTMLInputElement !== "undefined") {
	// stops errors occurring for browsers or JavaScript engines without the HTMLInputElement object
...
if(typeof console !== "undefined") {
	// stops errors occurring for browsers or JavaScript engines without the console object
...
function AFTime_FormatEx(cFormat) {
	...
	PDFevent.value = util.printd(cFormat, date, false);
}
